...
P, norm = cp.orth_ttr(n, dist, retall=True, normed=True)|\pause|

q0, q1 = cp.variable(2)|\pause|
P_nk = cp.outer(P, P)|\pause|
E_ank = cp.E(q0*P_nk, dist)|\pause|
E_ik = cp.E(q1*P, dist)|\pause|

def f(c_k, x):
  return -cp.sum(c_k*E_ank,-1)/norm
|\pause|
solver = odespy.RK4(f)|\pause|
c_0 = E_ik/norm|\pause|
solver.set_initial_condition(c_0)|\pause|
c_n, x_ = solver.solve(x)|\pause|
