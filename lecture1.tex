...
\pause
  \begin{align*}
    ...
    = \frac{1-   e^{-0.1x}}{10x} \\
    \onslide<4->{ Var(u) & =\int\limits_{-\infty}^{\infty} (u(x;a)- \E(u))^2p_a(a)da =
...
samples_a = dist_a.sample(num_samples=1000)
|\pause|
...
\begin{frame}{Coefficients are determined by least squares minimization}{}
    \begin{align*}
        \hat u_M &= \sum_{n=0}^N c_n P_n \\
        \onslide<2-> \min_{c_0,\ldots,c_N} || u- \hat u_M||_Q^2\\
          &\vdots\\
