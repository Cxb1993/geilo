...
\begin{frame}
 \frametitle{No.~of quadrature points $K$ grows exponentially with dimension $D$}
 % TODO
...
Some statistical metrics have anayltical formulas, others can easily be
implemented:

\begin{lstlisting}[language=python]
# Monte Carlo on the surrogate model

Q_samples = dist.samples(10**5)
u_samples = P(*Q_samples)
|\pause|
p_10 = np.percentile(u_sample, 10, axis=0)
p_90 = np.percentile(u_sample, 90, axis=0)
 \end{lstlisting}
\end{chaospy}
