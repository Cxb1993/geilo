...
\[   \varepsilon_E = \int_0^{10}|E(u) - E(\hat{u})|\,dx \qquad
 ...
 \begin{figure}
  \includegraphics[width=0.7\textwidth]{Convergence_repeat.png}
 \end{figure}
...
\pause
\begin{itemize}
    \item[$K$] Totoal number of quadrature nodes
    \item[$L$] Quadrature order along an axis
\end{itemize}
 \end{frame}

 \begin{frame}{Generating multivariate integration rules in
     Chaospy}{}
% TODO
...
\begin{frame}{Comparison between number of nodes using full tensor grid and sparse grid}{}
    ...
    % high: L vs K between GQ full tensor and GQ sparse grid
     \begin{figure}
  \includegraphics[width=0.6\textwidth]{dimensionality_nodes_gq_sparse.png}
 \end{figure}
\end{frame}
...
%  \end{frame}
 ...
 % in full tensor: >>> plt.ylim(10**-14, 10**2)
     \begin{columns}
         ...
         \begin{center}
                \includegraphics[width=1.1\textwidth]{convergence_2D_L_sparse.png}

                {Sparse grid: $L=M-1$, $L=M$}
         \end{center}
     \end{columns}
\end{frame}
...
\begin{frame}{A compare between different methods}{}
    % TODO
    % Plot!
...
\begin{chaospy}{Model approximation created; Now what?}
 \begin{lstlisting}[language=python]
