...
#for n in xrange(N):
    ...
    #errorPoly.append(dt*np.sum(np.abs(E_analytical(T) - cp.E(U_hat,dist))))
    #varPoly.append(dt*np.sum(np.abs(V_analytical(T) - cp.Var(U_hat,dist))))
